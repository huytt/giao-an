...
{
    public class ProductSTSController : Controller
    {
        //
        // GET: /ProductSTS/
        public ActionResult Index(int? page, int? pageSizeSelected)
        {
            ...
            ProductStatisticRepository _iProductStisticService = new ProductStatisticRepository();

            int pageNum = (page ?? pageNumDefault);
            int pageSize = (pageSizeSelected ?? (int)pageSizeDefault);
            ViewBag.page = page;
            ViewBag.pageSizeSelected = pageSize;
            List<ProductStatistic> list_Product = new List<ProductStatistic>();
            var tmp  = _iProductStisticService.GetList_ProductStatisticAll();

            foreach (var item in tmp)
            {
                if (list_Product.Where(_ => _.ProductId == item.ProductId).ToList().Count == 0)
                {
                    int ststscount = tmp.Where(x => x.ProductId == item.ProductId).ToList().Sum(_ => _.Counter) ?? 0;
                    int ststscounterMember = tmp.Where(x => x.ProductId == item.ProductId).ToList().Sum(_ => _.CounterMember) ?? 0;
                    ProductStatistic stst = new ProductStatistic();
                    stst.ProductStatisticId = item.ProductStatisticId;
                    stst.ProductId = item.ProductId;
                    stst.Counter = ststscount;
                    stst.CounterMember = ststscounterMember;
                    list_Product.Add(stst);
                }
            }

            var lst_ProductPage = list_Product.OrderByDescending(_=>_.Counter).ToPagedList(pageNum, pageSize);
           // ViewBag.lstProduct = _iProductService.GetList_ProductAll();
            ViewBag.SideBarMenu = "ProductIndex";
            return View(lst_ProductPage);
        }
        ...
        [HttpGet, WebMethod]
        public ActionResult GetProductStatistic2(string datefrom, string type, long? ProductId)
        {
            string result_csv = "[";
                ...
                switch (type)
                {
                    case "hour":
                        ...
                        for (int i = 0; i < 24; i++)
                        {
                            ...
                            int ststscount = lst_ProductSTS.Where(x => x.Date.Value.Day == date_from.Day && x.Date.Value.Year == date_from.Year && x.Date.Value.Month == date_from.Month && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => int.Parse(
                            ...
                            int ststscountMember = lst_ProductSTS.Where(x => x.Date.Value.Day == date_from.Day && x.Date.Value.Year == date_from.Year && x.Date.Value.Month == date_from.Month && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => int.Parse(
                                Common.Common.ToStatisticJson(_.Content).Where(p => p.TimeStart == i.ToString()).ToList().Count > 0 ?
                    ...
                    case "date":
                        ...
                        {
                            int ststscount = lst_ProductSTS.Where(x => x.Date.Value.Day == date_from.Day && x.Date.Value.Year == date_from.Year && x.Date.Value.Month == date_from.Month && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.Counter) ?? 0;
                            int countermember = lst_ProductSTS.Where(x => x.Date.Value.Day == date_from.Day && x.Date.Value.Year == date_from.Year && x.Date.Value.Month == date_from.Month && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.CounterMember) ?? 0;
                    ...
                    case "month":
                        ...
                        while (date_from != date_to && date_from < date_to)
                        {
                            int ststscount = lst_ProductSTS.Where(x => x.Date.Value.Month == date_from.Month && x.Date.Value.Year == date_from.Year && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.Counter) ?? 0;
                            int countermember = lst_ProductSTS.Where(x => x.Date.Value.Year == date_from.Year && x.Date.Value.Month == date_from.Month && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.CounterMember) ?? 0;
                            string label = ((date_from.Ticks - (new DateTime(1970, 1, 1)).Ticks) / 10000).ToString();
                    ...
                    case "year":
                        ...
                        while (date_from != date_to && date_from < date_to)
                        {
                            int ststscount = lst_ProductSTS.Where(x => x.Date.Value.Year == date_from.Year && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.Counter) ?? 0;
                            int countermember = lst_ProductSTS.Where(x => x.Date.Value.Year == date_from.Year && this.CheckProduct((long)x.ProductId, ProductId)).ToList().Sum(_ => _.CounterMember) ?? 0;
                            string label = ((date_from.Ticks - (new DateTime(1970, 1, 1)).Ticks) / 10000).ToString();
