...
public class BaoKimPayment
{
    ...
    internal string checkSum(string order_id, string transaction_id, string created_on, string payment_type, string transaction_status,
        string total_amount, string net_amount, string fee_amount, string merchant_id, string customer_name,
        string customer_email, string customer_phone, string customer_address)
    {
        Hashtable order_params = new Hashtable();
        order_params.Add("order_id", order_id);
        order_params.Add("transaction_id", transaction_id);
        order_params.Add("created_on", created_on);
        order_params.Add("payment_type", payment_type);
        order_params.Add("transaction_status", transaction_status);
        order_params.Add("total_amount", total_amount);
        order_params.Add("net_amount", net_amount);
        order_params.Add("fee_amount", fee_amount);
        order_params.Add("merchant_id", merchant_id);
        order_params.Add("customer_name", customer_name);
        order_params.Add("customer_email", customer_email);
        order_params.Add("customer_phone", customer_phone);
        order_params.Add("customer_address", customer_address);
        //Sắp xếp các tham số theo key để tiến hành mã hóa
        ICollection keyCollection = order_params.Keys;
        string[] keys = new string[keyCollection.Count];
        keyCollection.CopyTo(keys, 0);
    ...
    internal string checkSumMin(string order_id, string transaction_id, string merchant_id)
    {
        Hashtable order_params = new Hashtable();
        order_params.Add("order_id", order_id);
        order_params.Add("transaction_id", transaction_id);
